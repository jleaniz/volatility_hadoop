...
class AnalyticsEngine(object):
    ...
    def getVPNUnusualActivity(self):
        ...
        loginsByUser = self.sqlctx.sql(
            "select user, remoteip, count(*) as hits from vpn"
        )
        entries = loginsByUser.groupBy(loginsByUser.user).count().orderBy(desc('count')).limit(20).collect()
        ...
        for entry in entries:
            if entry.hits < 10:
                data.append(
                    [entry.remoteip, entry.user, entry.hits ]
                )

        data_table = gviz_api.DataTable(description)
        data_table.LoadData(data)
        # Creating a JSon string
        json = data_table.ToJSon(columns_order=("remoteip", "user", "hits"),
                                 order_by="hits")
