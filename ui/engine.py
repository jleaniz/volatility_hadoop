...
class AnalyticsEngine(object):
            ...
            if table == 'fw':
                parquetPaths.append(
                    '/data/srm/dbs/dw_srm.db/fw/date=%s%s%s' % (
                        day.year, str(int(str(day).split('-')[1])), str(day).split('-')[2])
                )
            else:
                parquetPaths.append(
    ...
    def pm_get_java_hosts(self, region, sft):
        ...
        hosts = self.sccmDF.filter('Region_X="{}"'.format(region.upper()))\
            .filter(self.sccmDF.Exclude_X.isNull())\
...
def init_spark_context():
    appConfig = conf.Config(exec_cores=8, cores_max=24, yarn_cores=2, instances=6, queue='root.llama')
